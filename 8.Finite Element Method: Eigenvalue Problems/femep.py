...
def trimesh(shape, n, m, mtype='basic', rho=None):
	...
	if mtype == 'advanced' and rho == None:
		...
		#rho = lambda x: (np.sin(np.pi * x / 2)**2 + eps) / (1 + eps)
	...
	def layer(n, shape, s0=0):
		...
		if shape == 'circle':
			...
			for s in np.linspace(0, 9/2, n, endpoint=False):
				if 0 <= s < 1/4:
					p = [s, 1/4]
				elif 1/4 <= s < 2/4:
					p = [1/4, 1/2 - s]
			...
			raise Exception(f'Shape {shape} has not been implemented yet.')
...
# Solve for the flow
def solve_eigs(cN, delD, cT, take_root=False):
	...
	B = np.zeros((len(cN), len(cN)))
	v = np.zeros((len(cN), len(cN)))

	# build matrices S and B
	for t in range(len(cT)):
		...
		for m in range(3):
			...
			for n in range(3):
				...
				# matrix B
				if m == n: B[cT[t,m],cT[t,n]] += tarea(t) / 6
				else: B[cT[t,m],cT[t,n]] += tarea(t) / 12
	...
	if take_root:
		...
		L = la.cholesky(B, lower=True)

		# construct new matrix
		Y = la.solve_triangular(L, S.T, lower=True)
		St = la.solve_triangular(L, Y.T, lower=True)

		# solve eigenvalue problem
		e, vt = la.eigh(St)

		# convert eigenvalues back
		vp = la.solve_triangular(L.T, vt) 

	else:
		e, vp = la.eigh(S, B)

	# add back Dirichlet zeros
	v[inds,:len(vp)] = vp

	return e, v

def draw_profile(cN, tri, c, shape, lmd, plot_type, savefig=False):
	fig, ax = plt.subplots(1)
	plt.title(fr'Eigenmode $\lambda={lmd}$, $N={len(cN)}$')
	...
	if plot_type == 'contourf':
		cnt = plt.tricontourf(cN[:,0], cN[:,1], tri.simplices, c, levels=200)
		clb = plt.colorbar(cnt, shrink=0.7, aspect=9)

	clb.ax.set_title(r'Amp. $A$')

	if savefig: plt.savefig(f'images/profile_{lmd}_{shape}_{plot_type}_{len(cN)}.png')

def analytic(mn, points):
	m, n = mn[0], mn[1]
	...
	for i in range(len(points)):
		...
		r = (x**2 + y**2)**(1/2)
		phi = np.arctan2(y, x)
		sol[i] = bes.jv(m, bes.jn_zeros(m, n)[-1] * r) * np.sin(m * phi)
...
for i in range(10):
	...
	'''
	sol = v[:,i]
	max_sol = np.max(np.abs(sol))
	sol = sol / max_sol

	anal = analytic(mns[i], points)
	
	max1 = np.max(np.abs(sol - anal))
	max2 = np.max(np.abs(sol + anal))
	if max1 < max2:
		draw_profile(points, tri, sol - anal, shape, w[i], 'contourf', savefig=True)
	else:
		draw_profile(points, tri, sol + anal, shape, w[i], 'contourf', savefig=True)
	'''
	plt.show()
...
for n in num_lst:
	...
	points, bound, tri = trimesh(shape, n, n, 'advanced')
	nodes.append(len(points))
	
	w, _ = solve_eigs(points, bound, tri.simplices)
	eigs.append(w[:10])
...
fit_f = lambda beta, x: beta[0] + beta[1] * (x + beta[2])**(beta[3])
xlabel = r'Number of Nodes $N_{N}$'
title = fr'Eigenvalue Extrapolation for Different Numbers of Nodes'
textbox = {'params': [0], 'symbols': [r'$\lambda^{\infty}$'], 'coords': [0.6, 0.5], 'error': True}

for i in range(10):
	ylabel = fr'Eigenvalue $\lambda_{i}$'
	savepath = f'images/eigenvalue_{i}_extrapolation_advanced.png'
	FitterPlotter(fit_f, [eigs[-1,i], -eigs[-1,i], -16, -2.0], nodes, eigs[:,i], nstd=1,
...
for n in num_lst:
	...
	start = timer()
	solve_eigs(points, bound, tri.simplices, take_root=False)
	end = timer()
	times_eigh.append(end - start)

	start = timer()
	solve_eigs(points, bound, tri.simplices, take_root=True)
	end = timer()
	times_chol.append(end - start)


print('nodes:', nodes)
print('Cholesky times:', times_chol)
print('Eigh times:', times_eigh)

xlabel = r'Number of Nodes $\ln(N_{N})$'
ylabel = r'Evaluation Time $\ln(t/[s])$'
title = fr'Evaluation Time for Different Numbers of Nodes'
ylim = [-6, 3.9]
savepath = 'images/times_nodes_fit_alt.png'
#textbox = {'params': [0], 'symbols': ['slope'], 'coords': [0.6, 0.3]}
fig, ax = FitterPlotter('lin', [1.0, 1.0], np.log(nodes), np.log(times_chol),
	nstd=1, title=title, xlabel=xlabel, ylabel=ylabel, ylim=ylim, ret_fig=True,
	datalabel='Cholesky', showfig=False)

FitterPlotter('lin', [1.0, 1.0], np.log(nodes), np.log(times_eigh), nstd=1,
	datalabel='Built-in', figax=(fig, ax), showfig=False)

FitterPlotter('lin', [1.0, 1.0], np.log(nodes[-35:]), np.log(times_eigh[-35:]),
	nstd=1, datalabel='last 35', figax=(fig, ax), savepath=savepath)
